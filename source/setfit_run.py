...
def run_labeling(filepath, run_gemini, only_gemini, log_callback, custom_labels, citation_threshold):
    def task():
        if only_gemini:
            topics = custom_labels.split(",") if custom_labels else []
            log_callback("Running Gemini classification (only)...")

            articles = geminiClassify(filepath, topics, citation_threshold)
            categoryCountDict = CategoryHandler.categoryCount(articles)
...
def create_ui():
    ...
    only_gemini_var = tk.BooleanVar()
    only_gemini_check = tk.Checkbutton(file_frame, text="Only Run Gemini", variable=only_gemini_var)
    only_gemini_check.pack(side=tk.LEFT, padx=5)
